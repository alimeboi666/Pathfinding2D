...
public class ResizeTexture2D : Utilities
{
    ...
    void OnGUI()
    {
        EditorGUILayout.LabelField("Click 1 ( or mutil Texture or choose the folder have them if them same visualSize) to Edit"); EditorGUILayout.LabelField("Change max Width, Height to crop");
        EditorGUILayout.LabelField("Resize return near to POT max of Texture");
        ...
        if (optionsChoose == Options.Crop)
        {
            for (int i = 0; i < listTexture2D.Count; i++)
            {
                ...
                if (texture != null)
                {
                    DrawTexture(listTexturesOrigin[i], i, 25, ": Origin visualSize: ");
                    DrawTexture(texture, i, defaultView + 55, "Preview visualSize: ");
                }
            }
        }
